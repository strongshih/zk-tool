...
static int sign_up(const char *name, const char *pubkey, const char *pass1, const char *pass2,
		const char *pass3, const char *pass4, const char *pass5, const char *pass6,
		const char *pass7, const char *pass8)
{
    ...
    str_printf(state.users[state.user_count].password[0], 16, "%s", pass1);
    str_printf(state.users[state.user_count].password[1], 16, "%s", pass2);
    str_printf(state.users[state.user_count].password[2], 16, "%s", pass3);
    str_printf(state.users[state.user_count].password[3], 16, "%s", pass4);
    str_printf(state.users[state.user_count].password[4], 16, "%s", pass5);
    str_printf(state.users[state.user_count].password[5], 16, "%s", pass6);
    str_printf(state.users[state.user_count].password[6], 16, "%s", pass7);
    str_printf(state.users[state.user_count].password[7], 16, "%s", pass8);
...
int contract_main(int argc, char **argv)
{
    ...
    if (str_cmp(argv[1], "sign_up", 7) == 0) {
        if (argc != 12) {
            err_printf("%s: usage: sign_up user_name user_pubkey 8-int-password\n", argv[0]);
            return 0;
        }

        int ret = sign_up(argv[2], argv[3], argv[4], argv[5], argv[6], argv[7],
